...
| 2026-06-01 | 0/6 | **Columna mal mapeada en la Unión** = la causa raíz que hizo fallar TODO el punto 6. El nombre `MOV_CREATED_DATE` traía datos de `MOV_OPERATION` (PAYMENT/REFUND). Al castear esa columna a "Fecha y hora", PAYMENT no parsea → **vacía todas las celdas** → toda función de fecha da T001. Además, una vez mal mapeada + casteada, la columna **se bloquea** (signo 🚫) y no se puede corregir. | Verificar el mapeo de CADA columna ANTES de guardar la unión (vista previa: dato coincide con nombre). No dejar columnas "sin mapear" (da error y rompe la unión). Si quedó rota y bloqueada → rehacer la unión. |
| 2026-06-01 | 6 | **MOV_CREATED_DATE viene en ISO** `2022-01-03T00:01:19-04:00` (T + timezone). Castear directo a Fecha y hora falla/vacía. | Extraer la fecha con `DIVIDIR(MOV_CREATED_DATE; "T"; 1)` → `2022-01-03` (limpio, sin hora), castear a Fecha, después `ADICIONAR_FECHA_TIEMPO(FECHA_LIMPIA; 1; "dias")`. |

| 2026-06-01 | 0b | **EL FORMATO DE UNA COLUMNA SE CAMBIA EN LA FUENTE, NO EN LA UNIÓN.** En la Unión las columnas heredadas dan "la columna seleccionada no admite cambio de formato". Castear `MOV_CREATED_DATE` a Texto EN LA FUENTE `DB_DOTA` hizo aparecer el dato. | Todo casteo/cambio de tipo → en la FUENTE. La Unión solo hereda. Regla para futuras pruebas: **nunca intentar castear en la unión.** |

> **Regla de oro reforzada:** cuando una función documentada da error de sintaxis en el workspace, NO inventar la sintaxis — verificar con el autocompletado del editor, buscar la función hermana en el Help Center, o preguntar al trainer. **Y cuando varias funciones distintas fallan igual sobre la misma columna → el problema es el DATO de esa columna (mapeo/formato), no las funciones.**
