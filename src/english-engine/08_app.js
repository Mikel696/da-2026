...
const NB = (() => {
  ...
  /* Solo cuando cambia QUÉ se enseña (otro cuaderno u otra página), y solo con la
     pestaña a la vista: desde Palabras también se crean cuadernos, y no puede
     moverte la página que estás leyendo. */
  let ultimaVista = null;
  function alInicio(b){
    const vista = b.id + ':' + b.cur;
    if(vista === ultimaVista) return;
    ultimaVista = vista;
    const pane = $('nb'), main = $('nbMain');
    if(!pane || !main || !pane.classList.contains('on')) return;
    const hh = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--hh')) || 56;
    const top = main.getBoundingClientRect().top;
    if(top < hh) window.scrollTo(0, Math.max(0, scrollY + top - hh - 12));
  }
...
const SONG = (() => {
  ...
  function wire(){
    ...
    const vv = $('sgVer');
    if(vv) vv.onclick = () => {
      P.verVideo = !P.verVideo;
      /* Sin render(): repintar el mando remonta el iframe y pararía el vídeo.
         Se cambia solo la clase que lo encoge — el audio sigue sonando. */
      const mando = document.querySelector('#sgFijo .sg-mando');
      const antes = mando ? mando.getBoundingClientRect().top : null;
      const caja = document.querySelector('#sgFijo .sg-yt');
      if(caja) caja.classList.toggle('oculto', !P.verVideo);
      vv.classList.toggle('on', P.verVideo);
      /* La página se acorta (o se alarga) de golpe y la barra de mandos se iría
         debajo de la cabecera, con este botón dentro. Se queda donde estaba. */
      if(mando && antes !== null){
        const desp = mando.getBoundingClientRect().top - antes;
        if(Math.abs(desp) > 2) window.scrollBy(0, desp);
      }
    };
